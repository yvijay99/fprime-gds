...
class YamcsClient(TransportClient):
    """YAMCS transport client for the GDS

    Telemetry and events are pushed via YAMCS WebSocket callbacks. Commands arrive as CmdData through the pipeline's
    command subscriber mechanism and are issued to YAMCS via REST, bypassing binary serialization. The _pending_cmd
    flag coordinates with send() to drop the redundant binary packet the encoder produces afterward.

    Note: implementation delegates to a YamcsWrapper
    """
